...
- 移动主题详情的收藏、Tweet、分享、忽略、感谢改为线性图标，标签与开启对话图标同步统一。
- 移动回复按钮与表情按钮同排显示。
...
- 移动主题列表标题置顶，下方合并节点、作者和时间，长标题自然换行，回复数与标题顶部对齐。
- 移动回复列表每条回复上下内边距增至12px。

## 1.13.48 更新

- 移动主题列表回复数量列按实际数字宽度占位，取消原生70px预留，与标题间距8px。
- 移动卡片左右内边距各增加5px至13px。

## 1.13.47 更新

- 移动主题列表每行上下留白增至16px；标题改为块级布局，上方6px、下方8px间距，增加文本区域占位。

## 1.13.46 更新

- 修复移动回复框嵌套身份区域的头像和 ID 隐藏规则，保留编辑、预览及工具按钮。
- 卡片上下内边距增至10px，回复框12px，卡片下方外边距16px。

## 1.13.45 更新

- 节点收藏使用四宫格图标，时间轴使用时钟图标，避免菜单图标重复。
- 移动每日活跃度改为顶部头像外围圆环，复用桌面进度和颜色，移除菜单横向进度条。
...
- 补齐移动菜单登录、注册等图标，统一菜单 14px、正文与回复 15px、主题标题 18px。
- 主题作者头像限定 30px，并给多行标题保留独立空间，避免遮挡。
- 修复首页分类区域上方圆角，分类导航改为单行横向滚动。
- 主题详情页右侧工具栏默认展开；扩展中已明确保存的关闭设置仍然有效。
